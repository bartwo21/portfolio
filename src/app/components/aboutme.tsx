    ...
    <div className="space-y-3">
      ...
      {highlights && (
        <div className="flex flex-wrap gap-2 pt-2">
          {highlights.map((h, i) => (
            <span key={i} className="px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-[11px] text-sky-100/70 font-bold uppercase tracking-widest">
              {h}
            </span>
          ))}
        </div>
...
const WorkspaceSectionComponent: React.FC<WorkspaceItem> = ({ title, items }) => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {items.map((item, itemIndex) => (
        ...
        >
          <span className="text-sky-100/70 text-sm font-medium">{item.name}</span>
          {item.link && (
            <a
              href={item.link}
              target="_blank"
...
const TechStackSectionComponent: React.FC<{
  ...
  <div className="space-y-8">
    <div className="flex items-center gap-4">
      ...
      <h3 className="text-sky-200/40 font-bold tracking-[0.3em] uppercase text-[11px]">{title}</h3>
      ...
      {skills.map((skill, index) => (
        ...
        >
          <div className="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center transition-all duration-500 group-hover:bg-white/[0.08] group-hover:border-white/20 group-hover:shadow-2xl">
            <skill.icon className={`text-2xl ${skill.color} opacity-60 group-hover:opacity-100 transition-opacity`} />
          </div>
          <span className="text-[10px] text-sky-100/40 font-bold uppercase tracking-widest text-center">{skill.name}</span>
        </motion.div>
      ))}
    </div>
  </div>
...
const Aboutme = () => {
  ...
  const [activeTab, setActiveTab] = useState<"journey" | "workspace" | "techstack">("journey");
      ...
      <div className="relative overflow-hidden rounded-[3rem] border border-white/10 bg-white/[0.02] backdrop-blur-3xl p-8 md:p-16 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
          <div className="space-y-4">
            <span className="text-sky-300/60 font-black tracking-[0.4em] uppercase text-xs">Curriculum Vitae</span>
            <h2 className="text-5xl md:text-7xl font-extralight tracking-tighter text-white">
              About <span className="font-serif italic text-sky-200">Bartu</span>
            </h2>
          </div>
          ...
          <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/10 backdrop-blur-sm">
            {["journey", "workspace", "techstack"].map((tab) => (
              <button
                ...
                className={`px-6 py-3 rounded-xl text-xs font-bold tracking-widest uppercase transition-all duration-500 ${activeTab === tab
          ...
          >
            {activeTab === "journey" ? (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                <div className="lg:col-span-3 space-y-8">
                  ...
                  <p className="text-sky-100/40 text-sm font-light leading-relaxed italic">
                    Building digital experiences with a focus on clean aesthetics and meaningful interactions.
                  </p>
                </div>
              ...
              <div className="space-y-20">
                <TechStackSectionComponent title="Primary Core" skills={techStackData.mainTech} />
                <TechStackSectionComponent title="Design & Visuals" skills={techStackData.styling} />
                <TechStackSectionComponent title="Infrastructure" skills={techStackData.backend} />
                <TechStackSectionComponent title="Workflow" skills={techStackData.tools} />
              </div>
            )}
          </motion.div>
        </AnimatePresence>
